...
async function fillWithData(orderId, input, label) {
    let dd = await runQuery(`SELECT *, testing_time as time FROM recipe_1_processing_details_dd WHERE orders_details_id = ${orderId} ORDER BY testing_time`);
    let slurry = await runQuery(`SELECT *, slurry_start_time as time FROM recipe_1_processing_details_slurry WHERE orders_details_id = ${orderId} ORDER BY slurry_start_time`);
    let outTest = await runQuery(`SELECT *, testing_time as time FROM recipe_1_out_test_during_production WHERE orders_details_id = ${orderId} ORDER BY testing_time`);
    // let outSemi = await runQuery(`SELECT *, bigbag_filling_time_end as time FROM recipe_1_out_semi_finished_production WHERE orders_details_id = ${orderId} ORDER BY testing_time`);

    let main = await runQuery(`SELECT finished.bigbag_filling_duration, finished.bigbag_filling_time_end, bigbag.sifter_speed_nominal_pct, semi.efficiency, avg(particles_grp1) as particles_grp1, avg(particles_grp2) as particles_grp2, avg(particles_grp3) as particles_grp3,avg(moisture) as moisture, line_dd
                                from recipe_1_orders_details order_details
                                    JOIN recipe_1_processing_details_bigbag bigbag ON bigbag.orders_details_id = order_details.id
                                    join recipe_1_out_semi_finished_production semi ON semi.bigbag_number = bigbag.bigbag_number AND semi.orders_details_id = order_details.id
                                    JOIN recipe_1_out_semi_finished_production finished ON finished.orders_details_id=order_details.id AND finished.bigbag_number=bigbag.bigbag_number
                                JOIN recipe_1_raw_material_in raw_in ON raw_in.id = order_details.id
                                JOIN recipe_1_raw_material_used raw_used ON raw_used.id = order_details.id AND raw_used.process_order_sap = raw_in.process_order_sap
                                 where order_details.id = ${orderId}
group by bigbag.orders_details_id, bigbag.bigbag_number, finished.bigbag_filling_duration, finished.bigbag_filling_time_end, bigbag.sifter_speed_nominal_pct, semi.efficiency, line_dd
   `);
    ...
    for (let row of main.recordset) {
        ...
        row.walec = (await runQuery(`SELECT AVG(steam_pressure_at_the_inlet_of_regulation_unit) as a, avg(product_temperature_at_the_outlet_of_JetCooker) as b, avg(setpoint_of_steam_pressure_at_the_DD_inlet) as c, avg(condensate_temperature_at_DD_outlet) as d, avg(product_temperature_at_the_inlet) as e, avg(setpoint_of_product_temperature) as f, avg(product_temperature_at_the_outlet_of_JetCooker) as g, avg(steam_pressure_at_the_outlet_of_regulation_unit) as i, avg(product_temperature_at_the_outlet_of_product) as j FROM [Walec ${row.line_dd}] WHERE timestamp between convert(varchar, '${startString}', 120) AND convert(varchar, '${endString}', 120)`)).recordset[0]
        if (row.dd.length == 0 || row.outTest.length == 0)
            continue;
        let inputArray = [
             row.particles_grp1, row.particles_grp2, row.particles_grp3, row.moisture,
            row.walec.a, row.walec.b, row.walec.c, row.walec.d, row.walec.e, row.walec.f,  row.walec.i, row.walec.j,
...
sql.connect(config, async function (err) {
    ...
    //     from recipe_1_raw_material_in raw_in
    //     join recipe_1_raw_material_used  raw_used ON raw_used.process_order_sap3 = raw_in.process_order_sap3 AND raw_used.id=raw_in.id
    //     ORDER BY `);
    let input = [];
    let label = [];
    let orders = await runQuery("SELECT * from recipe_1_orders_details WHERE data_split = 'training'");
    ...
    let ordersTest = await runQuery("SELECT * from recipe_1_orders_details WHERE data_split = 'test'");
    let inputTest = [];
    let labelTest = [];
